...
impl RpcServer {
    ...
    pub async fn start(&self) -> Result<()> {
        ...
        module.register_async_method("eth_getBlockByNumber", |_params, _catalog, _| async move {
            // TODO: Get block by number from RocksDB index
            Ok::<serde_json::Value, ErrorObjectOwned>(serde_json::json!(null))
        })?;

        module.register_async_method("eth_getTransactionByHash", |_params, _catalog, _| async move {
